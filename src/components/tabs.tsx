...
export interface TabsProps extends BaseProps {
  ...
  items?: string[] | { value: string, icon: ReactNode }[];
  tabTextSize?: "default" | "large";
  /**
...
export function Tabs({
  ...
  defaultIndex = 0,
  updateAnchor = false,
  tabTextSize: textSize = "default",
  ...props
}: TabsProps) {
  const values = useMemo(() => items.map((item) => 
    toValue(typeof item == "string" ? item : item.value)
), [items]);
  ...
  return (
    ...
    >
        ...
        {values.map((v, i) => {
          ...
          return (
            <Primitive.TabsTrigger key={v} value={v}>
              <span className={`${textSize === "large" ? "text-[1rem]" : ""} hover:cursor-pointer`}>
              {typeof item === "string"
                ? item
      ...
      <TabsContext.Provider
        value={useMemo(
          () => ({ items: itemList, valueToIdMap, collection }),
